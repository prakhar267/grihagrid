...
try {
  ...
  assert.equal(await page.getByLabel('Active floor',{exact:true}).inputValue(),'ground');
  assert.equal(await page.getByLabel('Active floor',{exact:true}).locator('option').count(),1);
  ...
  assert.equal(await page.getByRole('spinbutton',{name:'Overall width in metres'}).inputValue(),'12');
  assert.equal(await page.getByRole('spinbutton',{name:'Interior depth in metres'}).inputValue(),'10');
  findings.checks.push('Discarding a replacement model restores valid floor/room navigation, dimensions and tour stops');
  ...
  await fallback.getByRole('group',{name:'Editable building plan'}).waitFor();
