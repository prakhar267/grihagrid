...
| Production migration and deployment | Migration `0010` applied successfully and no migration remains pending. D1 exposes exactly two Family tables, seven triggers and three indexes. Worker version `13643004-0686-47e9-96b1-f4d836340ccb` is deployed; readiness reports `familyAlignmentSchema=current` and `familyAlignment=true` while `paidCheckout=false`, `acceptingPaidPlans=[]` and the ₹999 catalog item remains closed | The first immediate multi-colo smoke reached one pre-rollout readiness response; the canonical smoke passed after propagation at `2026-08-13T22:42:50Z` |
