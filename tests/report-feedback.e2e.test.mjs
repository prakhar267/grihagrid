...
test("report feedback is exact, private, immutable-report-safe, and observable on real D1", { timeout: 180_000 }, async () => {
  ...
  try {
    ...
    requireD1Success(d1(stateDirectory, "migrate"), "migrations 0013 through 0021 failed");
    ...
    assert.equal(migrationLedger.at(-1)?.name, "0021_professional_review_owner_listing.sql");
