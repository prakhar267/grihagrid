...
function stairLocal(stair,p){const dx=stair.end[0]-stair.start[0],dy=stair.end[1]-stair.start[1],length=Math.hypot(dx,dy),px=p[0]-stair.start[0],py=p[1]-stair.start[1];return {t:(px*dx+py*dy)/(length*length),side:(px*-dy+py*dx)/length,length,ux:dx/length,uy:dy/length}}
export function surfaceAt(scene,p,{eyeHeight=1650,clearance=220,maxStep=250}={}){const feet=Number.isFinite(p[2])?p[2]-eyeHeight:scene.floors[0].elevation,candidates=[];for(const stair of scene.stairs){const local=stairLocal(stair,p);if(local.t<0||local.t>1||Math.abs(local.side)>stair.width/2-clearance-35)continue;const from=floorFor(scene,stair.fromFloorId),to=floorFor(scene,stair.toFloorId),height=from.elevation+(to.elevation-from.elevation)*local.t;if(Math.abs(height-feet)>maxStep)continue;const floorId=local.t>.5?to.id:from.id;if(!legacyWalkable(floorScene(scene,floorId,{blockStairs:false}),p,clearance))continue;candidates.push({height,floorId,stairId:stair.id,difference:Math.abs(height-feet)})}
  for(const floor of scene.floors){if(Math.abs(floor.elevation-feet)>maxStep)continue;if(!legacyWalkable(floorScene(scene,floor.id,{blockStairs:false}),p,clearance))continue;if(floorApertures(scene,floor.id).some(h=>insidePolygon(p,h.polygon)))continue;if(scene.stairs.some(s=>s.fromFloorId===floor.id&&insidePolygon(p,stairPolygon(s),true)&&stairLocal(s,p).t>.001))continue;candidates.push({height:floor.elevation,floorId:floor.id,stairId:null,difference:Math.abs(floor.elevation-feet)})}
  return candidates.sort((a,b)=>a.difference-b.difference)[0]||null
}
export function getSurfaceHeight(scene,p,eyeHeight=1650){return surfaceAt(scene,p,{eyeHeight})?.height??null}
export function floorAtPosition(scene,p,eyeHeight=1650){const surface=surfaceAt(scene,p,{eyeHeight});return floorFor(scene,surface?.floorId)||null}
export function isWalkableV2(scene,p,clearance=220,eyeHeight=1650){return Boolean(surfaceAt(scene,p.length===2?[...p,scene.floors[0].elevation+eyeHeight]:p,{clearance,eyeHeight,maxStep:20}))}
export function isRouteClear(scene,path,{clearance=220,eyeHeight=1650}={}){if(!Array.isArray(path)||!path.length)return false;for(let i=0;i<path.length;i++){const a=path[Math.max(0,i-1)],b=path[i],n=Math.max(1,Math.ceil(Math.hypot(...a.map((v,j)=>v-b[j]))/60));for(let j=0;j<=n;j++){const p=a.map((v,k)=>v+(b[k]-v)*j/n);if(!surfaceAt(scene,p,{clearance,eyeHeight,maxStep:30}))return false}}return true}
export function resolveCollisionV2(scene,current,next,clearance=220,eyeHeight=1650){const move=end=>{let result=current.slice(),length=distance(current,end),steps=Math.max(1,Math.ceil(length/60));for(let i=1;i<=steps;i++){const p=[current[0]+(end[0]-current[0])*i/steps,current[1]+(end[1]-current[1])*i/steps,result[2]],surface=surfaceAt(scene,p,{clearance,eyeHeight,maxStep:220});if(!surface)return result;result=[p[0],p[1],surface.height+eyeHeight]}return result};const direct=move(next);if(distance(direct,next)<1)return direct;const x=move([next[0],current[1],current[2]]),y=move([current[0],next[1],current[2]]);return [direct,x,y].sort((a,b)=>distance(b,current)-distance(a,current))[0]}
export function getRoomAnchorV2(scene,roomId,clearance=220){const room=scene.rooms.find(r=>r.id===roomId);if(!room)return null;const projected=floorScene(scene,room.floorId),center=polygonCenter(room.polygon),candidate=legacyNearest(projected,center,{roomId,clearance,maxRadius:15000});if(!candidate)return null;return candidate}
