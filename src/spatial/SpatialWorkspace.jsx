...
export default function SpatialWorkspace({ projectId, onNavigate, logoutConfirmed = false }) {
  ...
  const [tab, setTab] = useState(()=>window.history.state?.initialView==='plan'?'plan':'explore'), [mode, setMode] = useState('overview');
  ...
  function regenerate(nextStops = stops, nextDuration = duration, source = 'manual', preferences=shotPreferences, height=eyeHeight) {
    ...
    try { const next = generateTour(model, {roomIds: nextStops, duration: Number(nextDuration),shotPreferences:preferences,eyeHeight:height});setTour({...next,source});setStops(next.roomIds);setDuration(next.duration);setShotPreferences(next.shotPreferences||preferences);setEyeHeight(next.eyeHeight||height);setTime(0);setPlaying(false);setRemote(current => current ? {...current,tourStale:false} : current);setMessage(source==='gemini'?'AI shot direction validated. Camera paths are calculated from the scene.':'Tour rebuilt from the current floor plan.');setError('');return next; } catch(e){setError(e.message);return null;}
  }
  ...
  function editModel(next){
    ...
    setStops(current=>{const kept=current.filter(id=>proposal.rooms.some(r=>r.id===id));return kept.length?kept:proposal.rooms.slice(0,12).map(r=>r.id);});
    ...
    setSelected(current=>proposal.rooms.some(r=>r.id===current)?current:proposal.rooms[0].id);
  ...
  function discardStudy() {
    ...
    const nextStops=retainedStops.length?retainedStops:tourStops.length?tourStops:next.rooms.slice(0,12).map(room=>room.id);
  ...
  const exportScene = () => download(new Blob([JSON.stringify({model,tour,viewpoints:currentViewpoints},null,2)],{type:'application/json'}),'grihagrid-scene.json');
  ...
  return <main className="sp-studio">
    ...
    <section className="sp-heading"><div><div className="sp-eyebrow"><span className="sp-dot"/> {projectId?'Project concept':'Interactive demonstration'} <span className="sp-heading-divider">/</span> {model.floors.length===1?'Single storey':`${model.floors.length} floors`}</div><h1>{projectId?remote.project.name:model.id===initial.id?'The Courtyard House':model.name}<span>.</span></h1><p>Edit the plan. Explore every room. Direct the camera tour.</p></div><div className="sp-heading-meta"><span>CONCEPT {String(model.revision).padStart(2,'0')}</span><strong>{model.rooms.reduce((a,r)=>a+area(r),0).toFixed(0)} <small>m²</small></strong><small>{model.rooms.length} spaces · {projectId&&remote.spatialRevision?'Saved concept':model.id===initial.id?'Sample geometry':'Reviewed geometry'}</small></div></section>
    {projectId&&!remote.model&&<p className="sp-first-model-note">Import a drawing or edit this sample, then review the concept before saving.</p>}
