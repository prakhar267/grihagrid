...
export default function SpatialWorkspace({ projectId, onNavigate, logoutConfirmed = false }) {
  ...
  const hasUnsavedChanges=briefDirty||Boolean(briefStudy)||dirty||pendingItinerary||tourDraftChanged||viewsDirty||Boolean(!projectId&&(model.revision!==initial.revision||views.length));
  useEffect(()=>{
  ...
  function editModel(next){
    ...
    setStops(current=>{const kept=current.filter(id=>proposal.rooms.some(r=>r.id===id));return kept.length?kept:defaultTourRoomIds(proposal);});
  ...
  return <main className="sp-studio">
    ...
    {projectId&&!remote.model&&<p className="sp-first-model-note">Your brief is saved. Open House brief to create a sized layout study, or import a drawing in 2D Plan. The example below is not your proposed house.</p>}
