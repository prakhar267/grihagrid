...
export default function CoordinationStudio({model,floorId,onSelectFloor,onChange,onStatus,onPendingChange,onExplore,disabled=false}) {
  ...
  useEffect(()=>{setDrafts(all=>reconcileComponentForms(scene,all))},[scene])
  ...
  function commit(event) {
    ...
    try {
      if(draft.conflict)throw new Error('This component changed outside the form. Load the current component or discard the form before applying changes.')
  ...
  function remove() {
    ...
    if(draft.conflict){fail(new Error('This component changed outside the form. Resolve the form conflict before removing it.'));return}
  ...
  function select(item) {
    ...
    setDiscipline(disciplineOf(item));setDraft(fromItem(scene.coordination.find(v=>v.id===item.id)));setPlacing(false);setError('');requestAnimationFrame(()=>formRef.current?.focus())
  }
  ...
  const numbers=(key,labels)=>labels.map((label,i)=><label key={label}>{label}<input aria-label={label} type="number" step="any" value={draft[key][i]} onChange={e=>change(key,draft[key].map((v,k)=>k===i?e.target.value:v))}/></label>)
  ...
  return <section className="cs-studio" aria-label="Structure and services authoring">
    ...
    <form ref={formRef} tabIndex={-1} className="cs-form" onSubmit={commit}><h3>{draft.id?'Edit component':'Add a component'}</h3><fieldset disabled={disabled}>
      ...
      <label>Rotation (degrees)<input aria-label="Component rotation" type="number" min="-360" max="360" step="any" value={draft.rotation} onChange={e=>change('rotation',e.target.value)}/></label>
      ...
      <div className="cs-actions"><button className="sp-primary" type="submit" disabled={Boolean(draft.conflict)}>{draft.id?'Apply component changes':'Add component to study'}</button><button type="button" onClick={()=>{setDraft(newItem(scene,floorId,draft.kind));setError('');setPlacing(false);requestAnimationFrame(()=>formRef.current?.focus())}}>{draft.pending?'Discard form changes':'New component'}</button>{draft.id&&<button type="button" disabled={Boolean(draft.conflict)} onClick={remove}>Remove component</button>}</div>
