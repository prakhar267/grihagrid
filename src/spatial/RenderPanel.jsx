...
export default function RenderPanel({ model, tour, viewpoints = [], disabled = false }) {
  ...
  async function disconnect() {
    ...
    pairedSession = null; setSession(null); setJobs([]); setError('')
    if (mediaUrl.current) URL.revokeObjectURL(mediaUrl.current)
  ...
  return <section className="render-panel" aria-label="Local render studio">
    ...
    {!session ? <>
      ...
      {(status === 'offline' || healthAttempt > 0) && <p role="status" className="render-note">{status === 'checking' ? 'Checking the local renderer…' : status === 'available' ? 'The local renderer is available. Enter its private pairing code to connect.' : 'The local renderer could not be reached. Check the setup above, then try again.'} <button type="button" aria-disabled={status === 'checking'} onClick={() => { if (status !== 'checking') { setError(''); setHealthAttempt(value => value + 1) } }}>Check renderer connection</button></p>}
    </> : <>
