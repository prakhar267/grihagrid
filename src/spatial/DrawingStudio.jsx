...
import { floorPlanSheet, elevationSheet, stairSectionSheet, openingSchedule, drawingSetHTML } from './drawing-set.js'
...
export default function DrawingStudio({model,floorId,northDegrees,children}) {
  const [view,setView]=useState('drawing'),[sheet,setSheet]=useState('plan'),[unit,setUnit]=useState('mm'),[zoom,setZoom]=useState(100)
  const options=useMemo(()=>({unit,northDegrees}),[unit,northDegrees])
  const svg=useMemo(()=>sheet==='elevations'?elevationSheet(model):sheet==='stairs'?stairSectionSheet(model):floorPlanSheet(model,floorId,options),[model,floorId,sheet,options])
  ...
  return <section className="drawing-studio" aria-label="Architectural drawings">
    ...
    <div hidden={view!=='drawing'}>
      <div className="ds-heading"><div><span className="sp-eyebrow">ARCHITECTURAL DRAWING SET</span><h2>A house, in detail.</h2><p>Measured plans, opening schedules, elevations and stair sections, coordinated with your 3D model.</p></div></div>
      <div className="ds-toolbar">
        <label>Drawing<select aria-label="Drawing sheet" value={sheet} onChange={e=>setSheet(e.target.value)}><option value="plan">Active floor plan</option><option value="elevations">Four elevations</option><option value="stairs">Stair sections</option></select></label>
