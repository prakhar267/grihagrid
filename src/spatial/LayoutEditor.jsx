import { furnitureSymbol } from './drawing-set.js'
import { furnitureKinds, furnitureSizes } from './furniture-catalog.js'
...
const kinds = furnitureKinds, sizes = furnitureSizes
...
export default function LayoutEditor({ model, onChange, onStatus, selectedRoomId, onSelectRoom, activeFloorId, onActiveFloorChange, disabled = false }) {
  ...
  return <section ref={editor} className="layout-editor" aria-label="Full layout editor" onKeyDown={key}><fieldset className="le-controls" disabled={disabled}>
    ...
    <div className="le-detail-action"><button type="button" disabled={disabled || !rooms.length} onClick={() => { const result = furnishRooms(scene, { floorId }); setFurnishReport({ floorId, added: result.added.length, skipped: result.skipped }); if (!result.added.length) { status(result.skipped.length ? result.skipped.join('; ') : 'This floor already has its suggested room details.'); return } commit(() => result.model, `Added ${result.added.length} editable furnishings. ${result.skipped.length ? `${result.skipped.length} item(s) need manual placement. ` : ''}Review Change Study before accepting.`) }}>Furnish this floor</button><span>Room furnishings and fixtures. Existing objects stay in place. Undo available.</span></div>{furnishReport?.floorId === floorId && <div className="le-furnish-report" role="status"><p>Last furnishing attempt: {furnishReport.added} objects added.</p>{furnishReport.skipped.length > 0 && <><p>These suggestions need manual placement using the Furniture tool:</p><ul>{furnishReport.skipped.map((note,i)=><li key={i}>{note}</li>)}</ul></>}</div>}<FloorTools scene={scene} floorId={floorId} selectedRoomId={selectedRoom?.id} onApply={commit} onSelectRoom={inspectRoom} onTool={chooseTool} disabled={disabled} onDrawStairs={(from, to) => { pendingTool.current = { floorId: from, tool: 'stairs' }; chooseFloor(from); setStairTarget(to); setTool('stairs'); window.requestAnimationFrame(() => svg.current?.focus()) }}/>
    ...
    <div className="le-editor-grid"><div><div className="le-plan-stage"><svg ref={svg} viewBox={box.join(' ')} role="group" aria-label="Editable building plan" tabIndex="0" onClick={click} onPointerMove={moving} onPointerUp={stopDrag} onPointerCancel={() => { drag.current = null; setPreview(null) }} onPointerDown={event => { if (tool === 'pan') beginDrag(event, { type: 'pan' }) }}>
      ...
      <g style={{ pointerEvents: 'none' }}>{display.furniture.filter(f => f.floorId === floorId).map(f => <g key={`symbol-${f.id}`} transform={`translate(${f.position[0]},${f.position[1]}) rotate(${f.rotation * 180 / Math.PI})`} dangerouslySetInnerHTML={{ __html: furnitureSymbol(f) }}/>)}</g>
