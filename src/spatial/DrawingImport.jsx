...
export default function DrawingImport({ model, onChange, onStatus, onCancel, blockedReason }) {
  ...
  return <section className="le-drawing" aria-label="Import and review a drawing">
    ...
    {blockedReason&&<p className="le-status" role="status">{blockedReason}</p>}
    ...
    {source && result && <>
      ...
      <label className="le-confirm"><input type="checkbox" checked={reviewed} onChange={event => setReviewed(event.target.checked)}/> I reviewed the detected rooms, openings, scale and construction assumptions against my drawing.</label><button type="button" className="le-primary" disabled={Boolean(blockedReason) || !reviewed || !mmPerPixel || !result.rooms.length || Boolean(busy)} onClick={apply}>Create model from reviewed geometry</button><p className="le-muted">This replaces the open spatial study after your review. Save through Change Study to create an accepted project revision.</p>
    </>}
