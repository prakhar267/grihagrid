...
function AccountLifecycleControls({user,disabled,onDeleted}){
  ...
  async function deleteAccount(event){
    ...
    try{
      await api('/api/account',{method:'DELETE',body:{currentPassword:deletion.password,confirmation:deletion.confirmation}});
      onDeleted();route('/');
    }catch(error){
      const code=error?.payload?.code;
      const blocked=code==="account_financial_retention_required";
      const message=blocked?'Deletion is paused because this account has financial records. Your account remains active; contact support for a governed retention review.':code==="current_password_incorrect"?'The current password is incorrect. Nothing was deleted.':'Account deletion was not completed. Your account remains active; retry when the connection is stable.';
