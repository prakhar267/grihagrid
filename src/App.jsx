...
const floorFactors = { G: .72, "G+1": 1.22, "G+2": 1.65, "G+3": 2.08 };
...
function useProjectCreationKey(recoverStored = false) {
  ...
  useEffect(() => {
    ...
    const deletionState=window.location.pathname==="/login"&&current?.accountDeleted===true?{accountDeleted:true,privateFileCleanupPending:current?.privateFileCleanupPending===true}:{};
    const safeState={projectCreationKey:key,...deletionState,...(continuation?{projectContinuation:true}:{}),...(continuation&&writeId&&revision?{anonymousDraftWriteId:writeId,anonymousDraftRevision:revision}:{}),...(estimatorSource?{estimatorSource}:{})};
...
function printArchitectPackOnly() {
  const root=document.documentElement;
  let finished=false;
  const finish=()=>{
    if(finished)return;
    finished=true;
    root.classList.remove("architect-export");
    window.removeEventListener("afterprint",finish);
  };
  root.classList.add("architect-export");
  window.addEventListener("afterprint",finish,{once:true});
...
function WorkspaceAccount({ user, onLogout }) {
  ...
  return <div className="workspace-account"><p>{user?.name||user?.email}</p><div className="workspace-account__actions">{["reviewer","admin"].includes(user?.accountRole)&&<button type="button" disabled={pending} aria-label="Professional review workbench" onClick={()=>route('/review-workbench')}><SealCheck/><span>Reviews</span></button>}<button type="button" disabled={pending} aria-label="Account security" onClick={()=>route('/security')}><LockKey/><span>Security</span></button><button ref={buttonRef} type="button" disabled={pending} aria-busy={pending} aria-describedby={failed?errorId:undefined} onClick={logout}><SignOut/><span>{pending?'Logging out…':failed?'Retry logout':'Log out'}</span></button></div><span className="workspace-account__status" role="status" aria-live="polite">{pending?'Logging out…':''}</span>{failed&&<p className="workspace-account__error" id={errorId} role="alert">{LOGOUT_FAILURE_MESSAGE}</p>}</div>;
}

const PUBLIC_ISSUES_URL = "https://github.com/prakhar267/grihagrid/issues";

function PublicSupportNotice() {
  return <div><p><a href={PUBLIC_ISSUES_URL} target="_blank" rel="noreferrer" referrerPolicy="no-referrer">Public bug reports on GitHub</a> require a GitHub account and are visible to everyone. Describe the steps and browser version without including account emails, passwords, private links, house drawings or site addresses. GrihaGrid does not attach your project or account data.</p><p>Private account support and email recovery are not configured for this release. Public issues cannot recover an account or handle private documents.</p></div>;
}
...
function Footer() {
  return <footer className="site-footer"><div className="footer-main">
    ...
    <div><h4>Help</h4><button onClick={() => route("/about")}>Studio guide</button><a href={PUBLIC_ISSUES_URL} target="_blank" rel="noreferrer" referrerPolicy="no-referrer">Public bug reports</a></div>
...
function StartPage({ user, draftAccess, onSessionEnded }) {
  ...
  async function createProject(){
    ...
    try {
      ...
      for(const file of uploadQueue){try{await api(`/api/projects/${result.project.id}/files`,{method:'POST',headers:{'content-type':file.type,'x-file-name':encodeURIComponent(file.name),'x-file-kind':'reference'},body:file})}catch{failed.push(file.name)}}
      if(!active.current)return;
...
function AuthPage({ mode, user, draftAccess, onAuthenticated }) {
  ...
  const deletionConfirmed=isLogin&&window.history.state?.accountDeleted===true;
  const deletionPending=deletionConfirmed&&window.history.state?.privateFileCleanupPending===true;
...
function PasswordRecoveryPage({mode}){
  ...
  useEffect(()=>{if(phase==="error"||phase==="success")window.requestAnimationFrame(()=>messageRef.current?.focus({preventScroll:true}))},[phase]);
  async function submit(event){
    event.preventDefault();setMessage("");
    if(confirming&&(!token||form.password.length<10||form.password.length>128||form.password!==form.confirm)){setPhase("error");setMessage(!token?'This recovery link is incomplete or expired.':form.password!==form.confirm?'The two password entries do not match.':'Use between 10 and 128 characters.');return}
    setPhase("pending");
    try{
      if(confirming)await api('/api/auth/password-reset/confirm',{method:'POST',body:{token,newPassword:form.password}});
      else await api('/api/auth/password-reset/request',{method:'POST',body:{email:form.email}});
...
function EmailVerificationPage({onVerified}){
  const [token]=useState(lifecycleTokenFromFragment);const [phase,setPhase]=useState(token?"pending":"error");const [message,setMessage]=useState(token?"":"This verification link is incomplete.");
  useEffect(()=>{if(!token)return;let active=true;api('/api/auth/email-verification/confirm',{method:'POST',body:{token}}).then(()=>{if(active){setPhase("success");onVerified?.()}}).catch(()=>{if(active){setMessage('This verification link is incomplete, expired, or already used.');setPhase("error")}});return()=>{active=false}},[token]);
  return <main className="shared-state"><Brand/><div role={phase==="error"?'alert':'status'}>{phase==="pending"?<ArrowClockwise/>:phase==="success"?<CheckCircle/>:<WarningCircle/>}<span className="kicker">Email verification</span><h1>{phase==="pending"?'Confirming your email…':phase==="success"?'Email verified.':'Link unavailable.'}</h1><p>{phase==="pending"?'The one-time link is being checked.':phase==="success"?'Recovery and important security messages can now use this address.':message}</p><button className="copper-button" disabled={phase==="pending"} onClick={()=>route('/security')}>Open account security <ArrowRight/></button></div></main>;
}
...
function AccountLifecycleControls({user,disabled,onDeleted}){
  const [verification,setVerification]=useState("idle");
  const [exportPhase,setExportPhase]=useState("idle");
  const [deletion,setDeletion]=useState({expanded:false,password:"",confirmation:"",phase:"idle",message:""});
  const messageRef=useRef(null);
  useEffect(()=>{if([verification,exportPhase,deletion.phase].includes("error")||deletion.phase==="blocked")window.requestAnimationFrame(()=>messageRef.current?.focus({preventScroll:true}))},[verification,exportPhase,deletion.phase]);
  async function requestVerification(){
    setVerification("pending");
    try{await api('/api/auth/email-verification/request',{method:'POST',body:{}});setVerification("sent")}
    catch(error){setVerification(error?.payload?.code==="email_unavailable"?"unavailable":"error")}
  }
  async function downloadExport(){
    ...
    try{
      ...
      const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download='grihagrid-account-export.json';link.click();URL.revokeObjectURL(url);setExportPhase("success");
    }catch{setExportPhase("error")}
  }
  async function deleteAccount(event){
    event.preventDefault();
    if(!deletion.password||deletion.confirmation!=="DELETE"){setDeletion(current=>({...current,phase:"error",message:'Enter your current password and type DELETE exactly.'}));return}
    setDeletion(current=>({...current,phase:"pending",message:""}));
    try{
      const result=await api('/api/account',{method:'DELETE',body:{currentPassword:deletion.password,confirmation:deletion.confirmation}});
      onDeleted();replaceRoute('/login',{accountDeleted:true,privateFileCleanupPending:result?.privateFileCleanup==='pending'});
    }catch(error){
      const code=error?.payload?.code;
      const blocked=code==="account_financial_retention_required";
      const message=blocked?'Deletion is paused because this account has financial records. Your account remains active; contact support for a governed retention review.':code==="current_password_incorrect"?'The current password is incorrect. Nothing was deleted.':code==='account_professional_offboarding_required'?'Professional reviewer accounts need operational offboarding before deletion. Your account remains active; contact support.':'Account deletion could not be confirmed. The request may have completed before the connection ended. Check your sign-in status before retrying.';
      setDeletion(current=>({...current,password:"",phase:blocked?"blocked":"error",message}));
    }
  }
  const busy=disabled||verification==="pending"||exportPhase==="pending"||deletion.phase==="pending";
  return <section className="security-lifecycle" aria-labelledby="account-lifecycle-title" aria-busy={busy}>
    <header><span className="kicker">Account lifecycle</span><h2 id="account-lifecycle-title">Keep control of the whole record.</h2><p>Verify recovery access, download a portable copy, or permanently close an eligible account.</p></header>
    <div className="security-lifecycle__actions">
      <article><div><strong>{user.emailVerified?'Email verified':'Verify your email'}</strong><p>{user.emailVerified?'Recovery and important security messages can use this confirmed address.':'GrihaGrid sends a one-hour confirmation link without putting its token in page history.'}</p></div><button type="button" className="outline-button" disabled={busy||user.emailVerified||verification==="sent"} onClick={requestVerification}>{verification==="pending"?'Sending…':verification==="sent"?'Link sent':user.emailVerified?'Verified':'Send verification link'}</button></article>
      <article><div><strong>Portable account export</strong><p>Download profile, projects, revisions, reports, feedback, order summaries and private-file metadata as JSON. Credentials and bearer tokens are excluded.</p></div><button type="button" className="outline-button" disabled={busy} onClick={downloadExport}><DownloadSimple/> {exportPhase==="pending"?'Preparing…':'Download my data'}</button></article>
      <article className="security-lifecycle__danger"><div><strong>Delete account</strong><p>Eligible accounts lose sessions, projects, reports, AI briefs and shares permanently. Uploaded files become inaccessible immediately; stored bytes are removed through a retryable cleanup process. Financial evidence, if any, requires a separate governed retention review.</p></div>{!deletion.expanded?<button type="button" className="outline-button" disabled={busy} onClick={()=>setDeletion({...deletion,expanded:true,phase:"idle",message:""})}><Trash/> Review deletion</button>:<form onSubmit={deleteAccount}><label>Current password<input required type="password" maxLength="128" autoComplete="current-password" disabled={busy} value={deletion.password} onChange={event=>setDeletion(current=>({...current,password:event.target.value,phase:"idle",message:""}))}/></label><label>Type DELETE<input required type="text" autoComplete="off" maxLength="6" disabled={busy} value={deletion.confirmation} onChange={event=>setDeletion(current=>({...current,confirmation:event.target.value,phase:"idle",message:""}))}/></label><div><button type="button" className="underlined-action" disabled={busy} onClick={()=>setDeletion({expanded:false,password:"",confirmation:"",phase:"idle",message:""})}>Cancel</button><button type="submit" className="outline-button" disabled={busy}>{deletion.phase==="pending"?'Deleting…':'Delete permanently'}</button></div></form>}</article>
    </div>
    {verification==="sent"&&<div className="security-message security-message--success" role="status"><CheckCircle/><div><strong>Verification link sent</strong><p>Open the newest message within one hour. Earlier unused links were closed.</p></div></div>}
    {verification==="unavailable"&&<div ref={messageRef} tabIndex="-1" className="security-message security-message--ambiguous" role="alert"><WarningCircle/><div><strong>Email unavailable</strong><p>Verification is temporarily closed. Your account and projects are unchanged.</p></div></div>}
    {(verification==="error"||exportPhase==="error"||deletion.message)&&<div ref={messageRef} tabIndex="-1" className={`security-message ${deletion.phase==="blocked"?'security-message--ambiguous':'security-message--error'}`} role="alert"><WarningCircle/><div><strong>{deletion.phase==="blocked"?'Retention review required':'Action not completed'}</strong><p>{deletion.message||(verification==="error"?'The requested account action could not be completed. Nothing was changed.':'The export could not be prepared. Retry when the connection is stable.')}</p></div></div>}
    {exportPhase==="success"&&<p className="security-form-status" role="status"><CheckCircle/> Account export downloaded.</p>}
  </section>;
}

function AccountSecurityPage({ user, onAuthenticated, onDeleted }) {
  ...
  return <main className="security-page"><header className="security-page__topbar"><Brand disabled={pending}/><button type="button" className="back-action" disabled={pending} onClick={()=>route('/dashboard')}><ArrowLeft/> My projects</button></header><div className="security-layout">
    ...
    <section className="security-panel" aria-label="Account security controls" aria-busy={pending}>
      ...
      <AccountLifecycleControls user={user} disabled={pending} onDeleted={onDeleted}/>
  ...
  return <main className={`project-home project-home--${archived?"archived":stale?"stale":stage}`}>
    ...
    <section className="project-evidence" aria-labelledby="project-evidence-title">
      ...
      <div className="project-evidence__grid">
        ...
        <EvidenceCard Icon={Sparkle} eyebrow="AI planning memo" tone={aiReady?"ready":aiAvailable?"attention":"optional"} title={aiReady?"Current AI reading":aiAvailable?"Earlier AI reading":"Optional reading not created"} copy={aiReady?"The saved advisory memo is tied to the current planning-report source.":aiAvailable?"This advisory memo belongs to an earlier planning-report source and is not presented as current.":"AI is an optional second reading. It never changes the deterministic range or professional boundary."} meta={aiBrief.generatedAt?`Generated ${formatDate(aiBrief.generatedAt)}`:null}/>
    ...
    <section className="project-home__danger" aria-labelledby="project-danger-title">
      <div><span className="kicker">Private project controls</span><h2 id="project-danger-title">Delete this project.</h2><p>This permanently removes an unpaid project record, its report, working comparisons, and Family Alignment rooms. The project must have no private file records and no purchase or payment evidence. Open the planning report to review any file records; files can be removed even during a storage outage, with byte cleanup retried automatically. Opening stored content requires private storage.</p></div>
...
function BriefEditor({ form, onChange, disabled=false }) {
  ...
  return <div className="brief-editor">
    ...
    <fieldset disabled={disabled}><legend><span>02</span><strong>Household programme</strong><small>Describe need, not a promised layout.</small></legend><div className="brief-editor__grid">
      <label>Floors<select required value={form.floors} onChange={event=>update("floors",event.target.value)}><option value="" disabled>Not stated</option>{["G","G+1","G+2","G+3"].map(value=><option key={value}>{value}</option>)}</select></label>
...
function scenarioSeed(project, index) {
  ...
  const alternatives = { G: "G+1", "G+1": "G+2", "G+2": "G+1", "G+3": "G+2" };
...
function ScenarioEditor({ scenario, index, onChange, disabled }) {
  ...
  return <fieldset className="scenario-editor" disabled={disabled}>
    ...
    <div className="scenario-editor__grid">
      <label>Floors<select value={scenario.floors} onChange={event=>update('floors',event.target.value)}>{['G','G+1','G+2','G+3'].map(value=><option key={value}>{value}</option>)}</select></label>
...
function ProjectFiles({ projectId, readonly = false }) {
  const [files,setFiles]=useState([]);const [busy,setBusy]=useState(false);const [error,setError]=useState("");const [removalMessage,setRemovalMessage]=useState("");const [storageState,setStorageState]=useState('loading');
  const privateUploads=usePrivateUploadCapability();
  ...
  async function upload(event){if(readonly||!privateUploads.enabled||storageState!=='ready'){event.target.value='';return}const selected=[...(event.target.files||[])].filter(file=>['image/jpeg','image/png','image/webp'].includes(file.type)&&file.size>0&&file.size<=10*1024*1024).slice(0,Math.max(0,20-files.length));if(!selected.length){setError('Choose a JPG, PNG or WebP image up to 10 MB.');event.target.value='';return}setBusy(true);setError("");try{for(const file of selected){const result=await api(`/api/projects/${projectId}/files`,{method:'POST',headers:{'content-type':file.type,'x-file-name':encodeURIComponent(file.name),'x-file-kind':'reference'},body:file});setFiles(current=>[result.file,...current]);}}catch(err){if(err.status===503){setStorageState('unavailable');setError('');return}setError(err.message);}finally{setBusy(false);event.target.value='';}}
  async function remove(file){const name=file.name||file.fileName||file.file_name||'this file';if(!window.confirm(`Delete “${name}”?`))return;try{setError('');const result=await api(`/api/projects/${projectId}/files/${file.id}`,{method:'DELETE',body:{}});setFiles(current=>current.filter(item=>item.id!==file.id));setRemovalMessage(result?.privateFileCleanup==='pending'?'Image removed from this project. Its stored bytes are queued for cleanup and cannot be opened.':'Image permanently deleted.');}catch{setError('Image removal could not be confirmed. Reload the file list before retrying.')}}
  ...
  return <section className={`project-files project-files--${checkingUploads?'loading':uploadsUnavailable?'unavailable':storageState} ${readonly?'project-files--readonly':''}`}><div><span className="kicker">Private site context</span><h2>Private plot photographs</h2><p>{readonly?'Existing verified images remain listed. Opening them requires storage; removal remains available during an outage and queues byte cleanup. Uploads are closed while this project is archived.':checkingUploads?'Existing verified images remain listed while upload availability is checked.':uploadsUnavailable?'Private image uploads are not active in this release. Your Brief Check and planning range remain available without them.':'Images are normalized to a static format, stripped of embedded metadata, and kept behind your account session.'}</p></div>{readonly?<span className="file-storage-state project-files__readonly"><LockKey/> Uploads closed</span>:checkingUploads?<span className="file-storage-state" role="status"><ArrowClockwise/> Checking upload availability…</span>:uploadsUnavailable?<span className="file-storage-state" role="status"><LockKey/> Uploads unavailable</span>:<label className="file-upload-action"><UploadSimple/>{busy?'Uploading…':'Add images'}<input disabled={busy} type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={upload}/></label>}{error&&<p className="form-error" role="alert">{error}</p>}{removalMessage&&<p className="security-form-status" role="status">{removalMessage}</p>}{files.length>0&&<div className="file-list">{files.map(file=>{const name=file.name||file.fileName||file.file_name||'Project image';return <div key={file.id}><FileText/><a href={`/api/projects/${projectId}/files/${file.id}`}>{name}</a><span>{Math.max(1,Math.round((file.sizeBytes||file.size_bytes||0)/1024))} KB</span><button onClick={()=>remove(file)} aria-label={`Delete ${name}`}><Trash/></button></div>})}</div>}</section>;
}
...
function AiPlanningBrief({ projectId, readonly = false }) {
  ...
  async function generate(){
    ...
    try{applyResult(await api(`/api/projects/${projectId}/ai-brief`,{method:'POST',body:{acceptedAiTerms:consented,aiProviders:aiProviders(allowGemini),refresh:hadAnalysis},timeoutMs:60_000}));}
    catch(err){
      if(err instanceof ApiError&&err.status===401){route('/login');return}
      if(isUnavailable(err)){
        setError(err.message);
        if(hadAnalysis){setError('AI generation is unavailable right now. Your saved brief is still available.');setPhase('ready')}
        else setPhase('unavailable');
        return;
      }
      setError(err.message||'AI could not create the brief. Please try again.');
  ...
  const generatedLabel=generatedDate&&!Number.isNaN(generatedDate.valueOf())
    ...
    :null;
  const model=String(analysis?.model||'AI').replace(/^models\//,'');
  ...
  return <section className={`ai-brief ${hasAnalysis?'ai-brief--has-analysis':''} ${readonly?'ai-brief--readonly':''}`} aria-labelledby="ai-brief-title" aria-busy={phase==='loading'||phase==='generating'||phase==='refreshing'}>
    <header className="ai-brief-heading">
      <div><span className="kicker">{analysis?.provider==='gemini'?'Google Gemini':'Cloudflare'} · AI-assisted</span><h2 id="ai-brief-title">A second reading of your brief.</h2><p>AI reads the saved planning report above—its assumptions may differ from your edited house model. The memo highlights priorities, risks and questions for your architect. Account details and project files are not sent.</p></div>
    ...
    {phase==='empty'&&(readonly?<div className="ai-brief-state ai-brief-state--muted"><LockKey/><div><span>Archived AI record</span><h3>No saved AI brief.</h3><p>This archived project has no AI planning memo. Generation is unavailable; the deterministic planning evidence above remains unchanged.</p></div></div>:<div className="ai-brief-state ai-brief-state--action"><Sparkle/><div><span>Optional planning layer</span><h3>Create an AI planning memo.</h3><p>Cloudflare Workers AI reads the planning facts in your report. You can separately allow Google Gemini as a fallback. Your saved memo shows which provider answered.</p><AiConsent checked={consented} onChange={setConsented} allowGemini={allowGemini} onGeminiChange={setAllowGemini}/><button className="copper-button" disabled={!consented} onClick={generate}>Generate AI brief <ArrowRight/></button><small>Usually ready in under a minute · output is advisory and saved with this project</small></div></div>)}

    {phase==='generating'&&<div className="ai-brief-state ai-brief-state--working" role="status" aria-live="polite"><Sparkle/><div><span>AI is reading your project</span><h3>Drafting the planning memorandum…</h3><p>This may take up to a minute. Keep this page open; no payment is involved.</p></div></div>}

    {phase==='unavailable'&&<div className="ai-brief-state ai-brief-state--muted" role="status"><WarningCircle/><div><span>AI studio unavailable</span><h3>AI generation is unavailable.</h3><p>{error||'The AI service may be unconfigured, at capacity or temporarily unavailable. You can keep working with the planning report.'}</p>{!readonly&&<><AiConsent checked={consented} onChange={setConsented} allowGemini={allowGemini} onGeminiChange={setAllowGemini}/><button className="outline-button" disabled={!consented} onClick={generate}>Try AI again <ArrowClockwise/></button></>}</div></div>}
    ...
    {hasAnalysis&&<div className="ai-brief-document">
      <div className="ai-brief-provenance"><span><i/> {readonly?'Archived brief · read only':cached?'Cached brief · saved privately':'New brief · saved privately'}</span><span>{generatedLabel?`Generated ${generatedLabel}`:'Saved AI brief'} · {analysis?.provider==='cloudflare'||model.startsWith('@cf/')?'Cloudflare Workers AI':'Google Gemini'} · {model}</span></div>
      {phase==='refreshing'&&<p className="ai-brief-refresh-status" role="status" aria-live="polite"><Sparkle/> AI is refreshing the memo. The saved version remains visible.</p>}
      {error&&<p className="ai-brief-inline-error" role="alert"><WarningCircle/>{error}</p>}
      <div className="ai-brief-overview"><span>Planning overview</span>{content.headline&&<h3>{content.headline}</h3>}<p>{content.overview||'AI has generated a planning brief for this project.'}</p></div>
      ...
      {!readonly&&<div className="ai-brief-actions"><div><p>Refresh after changing the project brief to create a new saved reading.</p><AiConsent checked={consented} onChange={setConsented} allowGemini={allowGemini} onGeminiChange={setAllowGemini} disabled={phase==='refreshing'}/></div><button className="outline-button" disabled={phase==='refreshing'||!consented} onClick={generate}>{phase==='refreshing'?'Refreshing…':'Refresh analysis'} <ArrowClockwise/></button></div>}
    </div>}
  </section>;
}
...
function ArchitecturalHandoffSections({ architecture, projectName=null, generatedAt=null, revisionLabel=null }) {
  const pack=normalizeArchitecturalHandoff(architecture);
  if(!pack)return null;
  const site=pack.siteBrief||{};
  const areas=pack.areaReconciliation||{};
  const requirements=pack.requirementRegister||[];
  const levelAreas=pack.levelAreaSchedule||[];
  const siteFacts=[
    ...
    ["Style direction",site.styleDirection||"Not shared / not stated"],
    ["Finish level",site.finishLevel||"Not stated"],
    ["Working budget",site.budgetLakh!==null&&site.budgetLakh!==undefined?`₹${Number(site.budgetLakh).toLocaleString("en-IN")} lakh`:"Not confirmed"],
  ];
  const areaRows=[
  ...
  return <section className="architect-pack" aria-labelledby="architect-pack-title">
    <header className="architect-pack__intro">
      <div><span className="kicker">{pack.stage}</span><h2 id="architect-pack-title">{pack.title}</h2>{projectName&&<div className="architect-pack__issue-meta"><strong>{projectName}</strong><span>{revisionLabel||`Pack v${pack.version}`}{generatedAt?` · ${formatDate(generatedAt)}`:""}</span></div>}<p>{pack.purpose}</p></div>
      <aside><Blueprint/><strong>Prepared for design</strong><span>Frozen brief → verified site → professional drawings</span></aside>
    </header>

    <section className="architect-pack__review-route" aria-label="Architecture design document workflow">
      <div><span>01</span><strong>Accept the brief</strong><p>Mark requirements accepted, commented or to be revised.</p></div>
      <div><span>02</span><strong>Replace assumptions</strong><p>Attach survey, title, statutory and utility evidence to the named registers.</p></div>
      <div><span>03</span><strong>Test one concept</strong><p>Issue plans, sections, area statement, outline specification and cost plan together.</p></div>
      <div><span>04</span><strong>Record departures</strong><p>Keep every room, area, budget or performance change in the decision register.</p></div>
    </section>
    ...
    {requirements.length>0&&<section className="architect-pack__section architect-pack__section--wide architect-pack__section--page" aria-labelledby="architect-requirements-title">
      <ArchitecturalSectionHeading eyebrow="B · Project requirements" title="A brief the architect can accept or challenge." copy="Priority, current basis and acceptance evidence are explicit so briefing decisions do not disappear into meeting notes." id="architect-requirements-title"/>
      <div className="architect-pack__table-wrap"><table className="architect-pack__table architect-pack__requirements-table"><thead><tr><th>Requirement</th><th>Priority</th><th>Current basis</th><th>Acceptance evidence</th></tr></thead><tbody>{requirements.map((item,index)=><tr key={`${item.code}-${index}`}><th><small>{item.code}</small><strong>{item.requirement}</strong></th><td><span className="architect-pack__status">{item.priority}</span></td><td>{item.currentBasis}</td><td>{item.acceptance}</td></tr>)}</tbody></table></div>
    </section>}

    <section className="architect-pack__section" aria-labelledby="architect-area-title">
      <ArchitecturalSectionHeading eyebrow="C · Area control" title="One arithmetic trail, no hidden area." copy="The scheduled programme and planning allowance reconcile to the target gross built-up area." id="architect-area-title"/>
      ...
      {levelAreas.length>0&&<><h3 className="architect-pack__subhead">Floor-by-floor reconciliation</h3><div className="architect-pack__table-wrap"><table className="architect-pack__table architect-pack__level-table"><thead><tr><th>Level</th><th>Gross target</th><th>Scheduled net</th><th>Residual</th><th>Test-fit status / action</th></tr></thead><tbody>{levelAreas.map((floor,index)=><tr key={`${floor.level}-${index}`}><th>{floor.level}</th><td>{Number(floor.grossTargetSqft||0).toLocaleString("en-IN")} sq ft</td><td>{Number(floor.scheduledNetSqft||0).toLocaleString("en-IN")} sq ft<br/><small>{Number(floor.netToGrossPercent||0).toLocaleString("en-IN")}% of gross</small></td><td className={Number(floor.residualSqft)<0?"architect-pack__negative":""}>{Number(floor.residualSqft||0).toLocaleString("en-IN")} sq ft</td><td><span className={`architect-pack__status architect-pack__status--${String(floor.status||"").toLowerCase().replace(/[^a-z]+/gu,"-")}`}>{floor.status}</span><p>{floor.action}</p></td></tr>)}</tbody></table></div></>}
    </section>

    <section className="architect-pack__section architect-pack__section--wide architect-pack__section--page" aria-labelledby="architect-room-title">
      <ArchitecturalSectionHeading eyebrow="D · Room data sheet" title="Room-by-room design targets." copy="Areas and nominal clear dimensions are starting points for coordination—not measured, statutory or construction dimensions." id="architect-room-title"/>
      ...
      {pack.rooms.some(room=>room.performance?.criticalCheck)&&<><h3 className="architect-pack__subhead architect-pack__subhead--page">Room performance schedule</h3><p className="architect-pack__subcopy">Furniture, environment and services must be demonstrated in the architect's concept—not left as unstated assumptions.</p><div className="architect-pack__table-wrap"><table className="architect-pack__table architect-pack__performance-table"><thead><tr><th>Room</th><th>Use and fit-out</th><th>Environment</th><th>Services and acceptance check</th></tr></thead><tbody>{pack.rooms.map((room,index)=><tr key={`performance-${room.code}-${index}`}><th><small>{room.code}</small><strong>{room.name}</strong></th><td><p>{room.performance?.useAndCapacity}</p><small>{room.performance?.fitOutAndStorage}</small></td><td>{room.performance?.environment}</td><td><p>{room.performance?.services}</p><strong>{room.performance?.criticalCheck}</strong></td></tr>)}</tbody></table></div></>}
    </section>

    <section className="architect-pack__section" aria-labelledby="architect-floor-title">
      <ArchitecturalSectionHeading eyebrow="E · Floor zoning" title="A level-by-level coordination brief." copy="This defines intent and vertical relationships; it deliberately stops short of inventing a floor plan." id="architect-floor-title"/>
    ...
    <section className="architect-pack__section" aria-labelledby="architect-adjacency-title">
      <ArchitecturalSectionHeading eyebrow="F · Planning logic" title="Adjacencies worth protecting." copy="Use these relationships to judge a proposed plan before debating finishes." id="architect-adjacency-title"/>
    ...
    <section className="architect-pack__section" aria-labelledby="architect-climate-title">
      <ArchitecturalSectionHeading eyebrow="G · Site and climate response" title={`Design moves to test in ${site.city||"the project climate"}.`} copy="The local professional must validate exposure, neighbours, air quality, drainage and adopted energy provisions." id="architect-climate-title"/>
    ...
    <section className="architect-pack__section architect-pack__section--page" aria-labelledby="architect-services-title">
      <ArchitecturalSectionHeading eyebrow="H · Structure and services" title="Coordinate the systems before they collide." copy="These are design intents and handoff responsibilities. Engineers must size and issue the technical work." id="architect-services-title"/>
    ...
    {pack.statutoryChecklist.length>0&&<section className="architect-pack__section architect-pack__section--wide" aria-labelledby="architect-statutory-title">
      <ArchitecturalSectionHeading eyebrow="I · Statutory due diligence" title="No rule is assumed to apply until it is cited." copy="The appointed architect completes this plot-specific schedule from current official instruments and records each dated source." id="architect-statutory-title"/>
      <div className="architect-pack__table-wrap"><table className="architect-pack__table architect-pack__statutory-table"><thead><tr><th>Control</th><th>Current basis</th><th>Professional check</th><th>Required evidence</th></tr></thead><tbody>{pack.statutoryChecklist.map((item,index)=><tr key={`${item.code}-${index}`}><th><small>{item.code}</small><strong>{item.topic}</strong><span className="architect-pack__status">{item.status}</span></th><td>{item.currentBasis}</td><td>{item.check}</td><td>{item.evidence}</td></tr>)}</tbody></table></div>
    </section>}

    <section className="architect-pack__section architect-pack__section--wide architect-pack__section--page" aria-labelledby="architect-verify-title">
      <ArchitecturalSectionHeading eyebrow="J · Verification register" title="What is known, what is assumed, who closes it." copy="A design item is not ready to freeze until its evidence, owner and decision gate are clear." id="architect-verify-title"/>
    ...
    {pack.consultantMatrix.length>0&&<section className="architect-pack__section architect-pack__section--responsibility" aria-labelledby="architect-responsibility-title">
      <ArchitecturalSectionHeading eyebrow="K · Responsibility matrix" title="One owner for every professional output." copy="Appointments and scopes remain contractual; this matrix prevents survey, structure, services and cost work from being silently omitted." id="architect-responsibility-title"/>
      <div className="architect-pack__table-wrap"><table className="architect-pack__table architect-pack__responsibility-table"><thead><tr><th>Role</th><th>Appoint by</th><th>Minimum responsibility</th><th>Expected issue</th></tr></thead><tbody>{pack.consultantMatrix.map((item,index)=><tr key={`${item.code}-${index}`}><th><small>{item.code}</small><strong>{item.role}</strong></th><td>{item.appointBy}</td><td>{item.responsibility}</td><td>{item.issue}</td></tr>)}</tbody></table></div>
    </section>}

    {(pack.decisionRegister.length>0||pack.stageGateChecklist.length>0)&&<section className="architect-pack__section architect-pack__section--wide architect-pack__section--page" aria-labelledby="architect-decisions-title">
      <ArchitecturalSectionHeading eyebrow="L · Decisions and acceptance" title="A review record, not another loose meeting note." copy="The architect records comments and departures here; client acceptance of the brief is not statutory or technical approval." id="architect-decisions-title"/>
      {pack.decisionRegister.length>0&&<div className="architect-pack__table-wrap"><table className="architect-pack__table architect-pack__decision-table"><thead><tr><th>Decision</th><th>Status / current position</th><th>Owner / needed by</th><th>Architect response</th></tr></thead><tbody>{pack.decisionRegister.map((item,index)=><tr key={`${item.code}-${index}`}><th><small>{item.code}</small><strong>{item.decision}</strong></th><td><span className={`architect-pack__status architect-pack__status--${String(item.status||"").toLowerCase().replace(/[^a-z]+/gu,"-")}`}>{item.status}</span><p>{item.currentPosition}</p></td><td><strong>{item.owner}</strong><small>{item.neededBy}</small></td><td className="architect-pack__response-line">Comment / departure / evidence reference</td></tr>)}</tbody></table></div>}
      {pack.stageGateChecklist.length>0&&<><h3 className="architect-pack__subhead">Stage gates</h3><div className="architect-pack__gates">{pack.stageGateChecklist.map((item,index)=><article key={`${item.code}-${index}`}><header><span>{item.code}</span><div><h3>{item.gate}</h3><strong>{item.status}</strong></div></header><p>{item.evidence}</p><small>{item.response}</small></article>)}</div></>}
      <div className="architect-pack__acceptance"><span className="kicker">Brief review record · not a statutory approval</span><div><p>Architect / practice / registration</p><p>Signature / date</p><p>Client decision-maker / date</p></div></div>
    </section>}

    <section className="architect-pack__section architect-pack__section--page" aria-labelledby="architect-drawings-title">
      <ArchitecturalSectionHeading eyebrow="M · Professional issue register" title="The drawings and checks still to be issued." copy="This list lets the client understand the architect’s next work and prevents a concept brief being mistaken for a drawing package." id="architect-drawings-title"/>
    ...
    <section className="architect-pack__section" aria-labelledby="architect-reference-title">
      <ArchitecturalSectionHeading eyebrow="N · Reference register" title="Current official controls must be retrieved and cited." copy="References identify where professional checking starts; they do not confirm that a clause applies to this plot." id="architect-reference-title"/>
...
function ReportFeedbackConcern({ unsaved=false }) {
  return <div className="report-feedback__concern" role="status"><WarningCircle/><p><strong>{unsaved?"Your concern was not saved—but do not rely on the item.":"Do not rely on a concerning item."}</strong> Take it to a licensed local professional. {unsaved?"The rejected response was not included in product learning and did not alert support.":"This structured response improves aggregate product learning but does not alert support."} For a product error, use <a href={PUBLIC_ISSUES_URL} target="_blank" rel="noreferrer" referrerPolicy="no-referrer">public bug reports on GitHub</a>. Reports are visible to everyone: include only generic steps and browser details, never account information, house drawings, private links or site addresses.</p></div>;
}
...
const professionalReviewStatusCopy={requested:"Waiting for a verified reviewer",assigned:"Review in progress",needs_owner_input:"Your response is needed",reviewed:"Professional review completed",cancelled:"Request cancelled"};

function ProfessionalReviewPanel({projectId,projectRevision,reportSchemaVersion,readonly=false}){
  const [phase,setPhase]=useState("loading");
  const [review,setReview]=useState(null);
  ...
  async function load(signal){
    setError("");setPhase("loading");
    try{
      const result=await api(`/api/projects/${encodeURIComponent(projectId)}/professional-reviews`,{signal});
      if(signal?.aborted)return;
      const current=(result.reviews||[]).find(item=>item.projectRevision===projectRevision&&item.reportSchemaVersion===reportSchemaVersion)||null;
      if(!current){setReview(null);setMessages([]);setPhase("ready");return}
      const detail=await api(`/api/projects/${encodeURIComponent(projectId)}/professional-reviews/${encodeURIComponent(current.id)}`,{signal});
      if(signal?.aborted)return;
      setReview(detail.review);setMessages(detail.messages||[]);setPhase("ready");
    }catch(err){if(signal?.aborted)return;if(err instanceof ApiError&&err.status===401){route('/login');return}setError(err?.message||"Professional review status could not be loaded.");setPhase("error")}
  }
  useEffect(()=>{const controller=new AbortController();load(controller.signal);return()=>controller.abort()},[projectId,projectRevision,reportSchemaVersion]);
  async function requestReview(event){event.preventDefault();if(readonly||busy)return;setBusy("request");setError("");const keyName=`grihagrid.professionalReview.${projectId}.${projectRevision}.${reportSchemaVersion}`;try{const result=await api(`/api/projects/${encodeURIComponent(projectId)}/professional-reviews`,{method:"POST",headers:{"idempotency-key":idempotencyKey(keyName)},body:{note}});removeSessionValue(keyName);setReview(result.review);setMessages([]);setNote("")}catch(err){setError(err?.payload?.code==="professional_review_active"?"This saved report already has an active review. Reload its status.":err?.message||"The review request was not created.")}finally{setBusy("")}}
  async function answer(event){event.preventDefault();if(!review||busy)return;setBusy("answer");setError("");try{await api(`/api/projects/${encodeURIComponent(projectId)}/professional-reviews/${encodeURIComponent(review.id)}/messages`,{method:"POST",body:{message:response}});setResponse("");await load()}catch(err){setError(err?.message||"Your response was not saved.")}finally{setBusy("")}}
  async function cancel(){if(!review||busy||!window.confirm("Cancel this unassigned professional review request?"))return;setBusy("cancel");setError("");try{await api(`/api/projects/${encodeURIComponent(projectId)}/professional-reviews/${encodeURIComponent(review.id)}`,{method:"DELETE",body:{}});await load()}catch(err){setError(err?.message||"The request was not cancelled.")}finally{setBusy("")}}
  return <section className="professional-review-panel" aria-labelledby="professional-review-title" aria-busy={phase==="loading"||Boolean(busy)}><header><div><span className="kicker">Professional review · pilot workflow</span><h2 id="professional-review-title">Put a verified practitioner on the record.</h2><p>This queue binds one reviewer, clarification thread, and completion note to this exact saved report. It is not municipal approval, structural certification, or permission to build.</p></div><SealCheck/></header>
    {phase==="loading"&&<p className="professional-review-panel__state" role="status"><ArrowClockwise/> Checking review status…</p>}
    {phase==="error"&&<p className="form-error" role="alert">{error}</p>}
    {phase==="ready"&&!review&&<form onSubmit={requestReview}><label>What should the reviewer challenge?<textarea rows="4" maxLength="1000" disabled={readonly||Boolean(busy)} value={note} onChange={event=>setNote(event.target.value)} placeholder="Optional context: access, programme pressure, assumptions, or a family concern."/></label><button className="copper-button" type="submit" disabled={readonly||Boolean(busy)}>{busy==="request"?"Joining pilot queue…":"Request pilot review"} <ArrowRight/></button>{readonly&&<small>This historical or archived report cannot start a new review.</small>}</form>}
    {phase==="ready"&&review&&<div className="professional-review-panel__record"><div className="professional-review-panel__status"><span>{professionalReviewStatusCopy[review.status]||review.status}</span><strong>Revision {review.projectRevision} · report v{review.reportSchemaVersion}</strong><small>Requested {formatDateTime(review.requestedAt)}</small></div>{review.ownerNote&&<blockquote>{review.ownerNote}</blockquote>}{review.reviewer&&<div className="professional-review-panel__reviewer"><SealCheck/><p><strong>{review.reviewer.displayName}</strong><span>{String(review.reviewer.discipline||"Professional").replaceAll("_"," ")} · {review.reviewer.licenseJurisdiction}</span><small>Reference {review.reviewer.licenseReference}</small></p></div>}{messages.length>0&&<ol className="professional-review-panel__messages">{messages.map(message=><li key={message.id}><strong>{message.authorRole==="reviewer"?"Reviewer":"You"}</strong><p>{message.body}</p><small>{formatDateTime(message.createdAt)}</small></li>)}</ol>}{review.status==="needs_owner_input"&&!readonly&&<form onSubmit={answer}><label>Respond to the open clarification<textarea required rows="4" minLength="1" maxLength="2000" disabled={Boolean(busy)} value={response} onChange={event=>setResponse(event.target.value)}/></label><button className="copper-button" disabled={Boolean(busy)}>{busy==="answer"?"Saving response…":"Send response"} <ArrowRight/></button></form>}{review.status==="reviewed"&&<div className="professional-review-panel__summary"><CheckCircle/><div><strong>Reviewer completion note</strong><p>{review.reviewerSummary}</p><small>Completed {formatDateTime(review.completedAt)}. This note stays attached to the exact report revision above.</small></div></div>}{review.status==="requested"&&!readonly&&<button className="underlined-action" type="button" disabled={Boolean(busy)} onClick={cancel}>{busy==="cancel"?"Cancelling…":"Cancel unassigned request"}</button>}{error&&<p className="form-error" role="alert">{error}</p>}</div>}
  </section>;
}

function ProfessionalReviewWorkbench({user}){
  const [reviews,setReviews]=useState([]);const [selected,setSelected]=useState(null);const [phase,setPhase]=useState("loading");const [busy,setBusy]=useState("");const [message,setMessage]=useState("");const [summary,setSummary]=useState("");const [error,setError]=useState("");
  async function load(){setError("");setPhase("loading");try{const result=await api('/api/professional-reviews');setReviews(result.reviews||[]);setPhase("ready")}catch(err){if(err instanceof ApiError&&err.status===401){route('/login');return}setPhase(err instanceof ApiError&&err.status===403?"forbidden":"error");setError(err?.message||"The reviewer queue could not be opened.")}}
  useEffect(()=>{load()},[]);
  async function open(review){setBusy(review.id);setError("");try{const result=await api(`/api/professional-reviews/${encodeURIComponent(review.id)}`);setSelected(result)}catch(err){setError(err?.message||"This review could not be opened.")}finally{setBusy("")}}
  async function claim(review){setBusy(review.id);setError("");try{await api(`/api/professional-reviews/${encodeURIComponent(review.id)}/claim`,{method:"POST",body:{}});await load();const result=await api(`/api/professional-reviews/${encodeURIComponent(review.id)}`);setSelected(result)}catch(err){setError(err?.message||"This review could not be claimed.")}finally{setBusy("")}}
  async function ask(event){event.preventDefault();if(!selected||busy)return;setBusy("message");setError("");try{await api(`/api/professional-reviews/${encodeURIComponent(selected.review.id)}/messages`,{method:"POST",body:{message}});setMessage("");setSelected(await api(`/api/professional-reviews/${encodeURIComponent(selected.review.id)}`));await load()}catch(err){setError(err?.message||"The clarification was not sent.")}finally{setBusy("")}}
  async function complete(event){event.preventDefault();if(!selected||busy)return;setBusy("complete");setError("");try{const result=await api(`/api/professional-reviews/${encodeURIComponent(selected.review.id)}`,{method:"PUT",body:{summary}});setSummary("");setSelected({...selected,review:result.review});await load()}catch(err){setError(err?.message||"The completion note was not saved.")}finally{setBusy("")}}
  if(phase==="loading")return <main className="review-workbench review-workbench--state"><header><Brand/><button onClick={()=>route('/dashboard')}><ArrowLeft/> Workspace</button></header><section role="status"><ArrowClockwise/><h1>Opening the reviewer queue…</h1></section></main>;
  if(phase==="forbidden")return <main className="review-workbench review-workbench--state"><header><Brand/><button onClick={()=>route('/dashboard')}><ArrowLeft/> Workspace</button></header><section><LockKey/><span className="kicker">Verified practitioners only</span><h1>This account has no active reviewer profile.</h1><p>Reviewer role and license evidence are provisioned through the controlled operations workflow—not through public registration.</p></section></main>;
  return <main className="review-workbench"><header><Brand/><div><span>{user?.name||"Professional reviewer"}</span><button onClick={()=>route('/dashboard')}><ArrowLeft/> Workspace</button></div></header><section className="review-workbench__intro"><span className="kicker">Professional review · controlled pilot</span><h1>Challenge the evidence.<br/><i>Record the boundary.</i></h1><p>Claim one exact report, ask a traceable clarification, and leave a completion note. Never treat this workflow as approval, structural design, sanction, or a substitute for the project’s appointed professionals.</p></section>{error&&<p className="form-error review-workbench__error" role="alert">{error}</p>}<div className="review-workbench__layout"><aside aria-label="Professional review queue"><header><h2>Queue & assigned work</h2><span>{reviews.length} record{reviews.length===1?"":"s"}</span></header>{reviews.length===0?<p>No review is waiting or assigned to you.</p>:reviews.map(review=><article key={review.id} className={selected?.review?.id===review.id?"active":""}><span>{professionalReviewStatusCopy[review.status]||review.status}</span><strong>Report revision {review.projectRevision}</strong><p>{review.ownerNote||"No owner note supplied."}</p><small>Requested {formatDateTime(review.requestedAt)}</small>{review.status==="requested"?<button className="copper-button" disabled={Boolean(busy)} onClick={()=>claim(review)}>{busy===review.id?"Claiming…":"Claim review"}</button>:<button className="outline-button" disabled={Boolean(busy)} onClick={()=>open(review)}>{busy===review.id?"Opening…":"Open record"}</button>}</article>)}</aside><section className="review-workbench__record">{!selected?<div className="review-workbench__empty"><FileText/><h2>Select or claim a review.</h2><p>The owner identity is withheld from the queue. The assigned record exposes only the saved project report and its clarification thread.</p></div>:<><header><span className="kicker">Assigned evidence · revision {selected.review.projectRevision}</span><h2>{selected.project.name}</h2><p>{professionalReviewStatusCopy[selected.review.status]||selected.review.status} · report schema v{selected.review.reportSchemaVersion}</p></header>{selected.review.reviewer&&<p className="review-workbench__license"><SealCheck/> Acting as {selected.review.reviewer.displayName} · {selected.review.reviewer.licenseJurisdiction} · {selected.review.reviewer.licenseReference}</p>}<section className="review-workbench__report"><h3>{selected.report.title||"Saved planning report"}</h3><p>{selected.report.briefCheck?.summary||selected.report.summary?.verdict}</p><h4>Risks to challenge</h4><ul>{(selected.report.risks||[]).map((risk,index)=><li key={`${risk}-${index}`}>{risk}</li>)}</ul><h4>Recorded next actions</h4><ol>{(selected.report.nextActions||[]).map((action,index)=><li key={`${action}-${index}`}>{action}</li>)}</ol></section>{(selected.messages||[]).length>0&&<ol className="professional-review-panel__messages">{selected.messages.map(item=><li key={item.id}><strong>{item.authorRole==="reviewer"?"Reviewer":"Owner"}</strong><p>{item.body}</p><small>{formatDateTime(item.createdAt)}</small></li>)}</ol>}{selected.review.status==="assigned"&&<div className="review-workbench__actions"><form onSubmit={ask}><label>Ask one clarification<textarea required rows="4" maxLength="2000" value={message} onChange={event=>setMessage(event.target.value)}/></label><button className="outline-button" disabled={Boolean(busy)}>{busy==="message"?"Sending…":"Request owner response"}</button></form><form onSubmit={complete}><label>Completion note<textarea required rows="7" minLength="20" maxLength="4000" value={summary} onChange={event=>setSummary(event.target.value)} placeholder="Record what was reviewed, what remains unverified, and which appointed professionals must validate the next stage."/></label><button className="copper-button" disabled={Boolean(busy)}>{busy==="complete"?"Saving note…":"Complete review"} <SealCheck/></button></form></div>}{selected.review.status==="needs_owner_input"&&<p className="professional-review-panel__state"><ArrowClockwise/> Waiting for the owner’s response before completion.</p>}{selected.review.status==="reviewed"&&<div className="professional-review-panel__summary"><CheckCircle/><div><strong>Completion note</strong><p>{selected.review.reviewerSummary}</p><small>Completed {formatDateTime(selected.review.completedAt)}</small></div></div>}</>}</section></div></main>;
}
...
function ReportPage({ id, revision=null }) {
  ...
  return <main className={`report-page ${archived?"report-page--archived":""} ${historical?"report-page--historical":""}`}><header><button onClick={()=>route(historical?`/projects/${id}/brief`:`/projects/${id}`)}><ArrowLeft/> {historical?"Brief history":"Project home"}</button><Brand/><div className="report-print-actions"><button onClick={()=>window.print()}><DownloadSimple/> Full report</button>{architecturalHandoff&&<button onClick={printArchitectPackOnly}><Blueprint/> Architecture design doc</button>}</div></header><div className="report-document">
    ...
    {architecturalHandoff && (
      <ArchitecturalHandoffSections architecture={architecturalHandoff} projectName={reportTitle} generatedAt={report.generatedAt} revisionLabel={historical?`Revision ${revision} · report schema v${reportSchemaVersion}`:`Project revision ${projectRevision} · report schema v${reportSchemaVersion}`}/>
    )}
    ...
    {Number.isInteger(projectRevision)&&projectRevision>0&&reportSchemaVersion===2&&<ProfessionalReviewPanel key={`professional-review:${id}:${projectRevision}:${reportSchemaVersion}`} projectId={id} projectRevision={projectRevision} reportSchemaVersion={reportSchemaVersion} readonly={archived||historical}/>}
